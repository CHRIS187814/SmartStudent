...
def complete_task(task_id: str, current_user=Depends(get_current_user)):
    client = _require_supabase()
    try:
        client.table("tasks").update({"status": "Completed", "priority_score": 0.0}).eq("id", task_id).eq(
            "user_id", str(current_user.id)
        ).execute()
        logger.info("Task completed for user_id=%s task_id=%s", current_user.id, task_id)
        return {"status": "success"}
    except Exception:
        logger.exception("Task completion failed for user_id=%s task_id=%s", current_user.id, task_id)
